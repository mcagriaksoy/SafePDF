...
class SafePDFController:
    ...
    def execute_operation_async(self, output_path=None, output_dir=None):
        ...
        thread = Thread(target=self._run_operation_thread, args=(output_path, output_dir), daemon=True)
        thread.start()
        ...
        try:
            ...
            if license_files:
                ...
                for idx, license_file_path in enumerate(license_files, 1):
                    self.logger.info(
                        f"[STARTUP] Verifying license {idx}/{len(license_files)}: {os_path.basename(license_file_path)}"
                    )
                    ...
                    if success:
                        ...
                        self.pro_expiry_date = expiry_date
                        self.logger.info("[STARTUP] ✓ License verified successfully!")
                        ...
                        self.logger.info("[STARTUP] ✓ Pro features ACTIVATED")
                        return
                    else:
            ...
            self.logger.info("[STARTUP] Running in FREE mode")
            self.is_pro_activated = False
            self.activation_key = None
            self.pro_expiry_date = None
            ...
            self.pro_expiry_date = None
            self.activation_key = None
            self.logger.info("[STARTUP] Running in FREE mode (error occurred)")
